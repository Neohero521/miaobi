...
// ─── 续写长度枚举 ──────────────────────────────────────────────────────────────
enum class WriteLength(val tag: String, val wordCount: Int) {
    SHORT("short", 300),
    MIDDLE("middle", 600),
    LONG("long", 1000)
}
...
sealed class EditorLoadState {
    object Idle : EditorLoadState()
    object Loading : EditorLoadState()
    data class Error(val msg: String) : EditorLoadState()
    data class Success(val result: String) : EditorLoadState()
}
